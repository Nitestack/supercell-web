...
if (leagueStatistics == "true")
    createCountdown(document.getElementById("crSeasonTimer"), getClashRoyaleSeasonEnd(), false, true);
setModule("profile");
...
function profileClick() {
    if (isMobileOrTablet())
        window.open(document.getElementById("shareButton").getAttribute("data-mobile-link"));
    /* Copy the text inside the text field */
    //@ts-ignore
...
function clanClick() {
    if (isMobileOrTablet())
        window.open(document.getElementById("clanProfile").getAttribute("data-mobile-link"));
    /* Copy the text inside the text field */
    //@ts-ignore
