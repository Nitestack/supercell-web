...
function setBoostModal(title, description, imageSrc, additionalDescription) {
    ...
    //@ts-ignore
    if (!additionalDescription)
        document.getElementById("boostModalAmountInput").value = "1";
    if (!additionalDescription)
        document.getElementById("boostModalAmount").textContent = (1).toString();
    ...
    if (!additionalDescription)
        document.getElementById("boostModalImage").src = imageSrc;
}
;
function openClockTowerBoostModal(playerTag, freeBoost) {
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/getBuilders",
        data: {
            village,
            playerTag,
        ...
        success: function ({ builders, boostDuration }) {
            $.ajax({
                method: "POST",
                url: "/upgrade-tracker/getLab",
                data: {
                    village,
                    playerTag
                },
                success: function (lab) {
                    setBoostModal(freeBoost ? "Free Boost!" : "Boost Builder Base?", `Collect resources, research and build 10x faster for ${freeBoost ? shortener(convertTime(boostDuration)) : "30m"}!`, `/Images/Clash of Clans/Magic Items/Clock Tower ${freeBoost ? "Boost" : "Potion"}.png`, freeBoost ? "You can boost for free once per 22h!" : null);
                    ...
                    document.getElementById("applyBoostButton").onclick = function () {
                        if (document.getElementById("boostModalAmount").textContent == "0" && !freeBoost)
                            return;
                        $.ajax({
                            method: "POST",
                            url: "/upgrade-tracker/applyBoost",
                            data: {
                                playerTag: playerTag,
...
;
function openBuilderPotionModal(playerTag) {
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/getBuilders",
        data: {
        ...
        success: function (builders) {
            ...
            document.getElementById("applyBoostButton").onclick = function () {
                if (document.getElementById("boostModalAmount").textContent == "0")
                    return;
                $.ajax({
                    method: "POST",
                    url: "/upgrade-tracker/applyBoost",
...
;
function openResearchPotionModal(playerTag) {
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/getLab",
        data: {
...
;
function setBoostTable(builders, amount, boost, freeBoost) {
    ...
    $(title).fadeOut("fast", function () {
        ...
        if (endTitle.toLowerCase().includes("h"))
            titleArray.splice(titleArray.length - 1, 1, `${amount}h?`);
        //Clock Tower
        else if (endTitle.toLowerCase().includes("m") && !freeBoost)
            titleArray.splice(titleArray.length - 1, 1, `${amount * 30}m!`);
...
if (document.getElementById("update").value == "true" ? true : false) {
    ...
    function setIntervalREQ() {
        $.ajax({
            method: "POST",
            url: "/upgrade-tracker/updateTimers",
            data: {
                village,
                //@ts-ignore
            ...
            success: function ({ builder, lab, petHouse, htmlCode }) {
                if (htmlCode) {
                    ...
                    document.getElementById(`${["heroes", "troops", "darktroops", "spells", "siegemachines"].includes(hash.toLowerCase()) ? "heroAndTroop" : "builder"}UpgradeDone`).play();
                    const modules = document.getElementById("modules");
                    $(modules).fadeOut("fast", function () {
                        modules.innerHTML = htmlCode;
                        setModule(location.hash.replace(/#/g, ""));
...
function startBuilderUpgrade(buildingID, buildingName, id, currentLevel, playerTag) {
    ...
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/startBuilderUpgrade",
        data: {
            buildingName,
            id,
            currentLevel,
...
;
function startLaboratoryUpgrade(unitName, currentLevel, playerTag) {
    const buttonParent = $(`button.lab-button[data-id=${unitName.replace(/\./g, "\\.").replace(/ /g, "\\ ")}]`)[0].parentElement;
    const { innerHTML } = buttonParent;
    buttonParent.innerHTML = loadingScreen;
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/startLaboratoryUpgrade",
        data: {
            unitName,
            currentLevel,
            village,
        ...
        success: function (result) {
            ...
            if (result.errorMessage) {
                ...
                document.getElementById('startLaboratoryUpgrade').play();
                const modules = document.getElementById("modules");
                $(modules).fadeOut("fast", function () {
                    modules.innerHTML = result.htmlCode;
                    setModule(location.hash.replace(/#/g, ""));
...
;
function upgradeWall(currentLevel, village, playerTag) {
    const input = document.getElementById("inputWall" + currentLevel);
    //@ts-ignore
    const amount = parseInt(input.value);
    //@ts-ignore
    if (amount < 1 || amount > parseInt(input.max))
        return;
    $.ajax({
        method: "POST",
        url: "/upgrade-tracker/upgradeWalls",
        data: {
            village,
            currentLevel,
            amount,
        ...
        success: async function (result) {
            if (result.errorMessage) {
                ...
                document.getElementById("builderUpgradeDone").play();
                const modules = document.getElementById("modules");
                $(modules).fadeOut("fast", function () {
                    modules.innerHTML = result.htmlCode;
                    setModule(location.hash.replace(/#/g, ""));
...
;
document.getElementById("scrollBar").scrollTop = 1;
let lastScrollTop = 1;
document.getElementById("scrollBar").onscroll = function (ev) {
    //@ts-ignore
    if (lastScrollTop == 1 && 1 > this.scrollTop)
        this.scrollTop = 1;
    //@ts-ignore
...
function shortener(text) {
    ...
    for (const textPiece of textArray)
        if (["0m", "0s", "0ms"].includes(textPiece))
            textArray.splice(textArray.indexOf(textPiece), 1);
    if (!text.toLowerCase().includes("d") && !text.toLowerCase().includes("h") && !text.toLowerCase().includes("m"))
        return "1m";
    return textArray.slice(0, text.toLowerCase().includes("d") ? 3 : (!text.toLowerCase().includes("h") ? 1 : 2)).join(" ");
}
;
function timeToGems(timeInSeconds) {
    var r, u, i, f, gems;
