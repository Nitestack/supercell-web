...
for (const element of ["cwl", "clanGames", "cocSeasonEnd", "cocLeagueReset", "bsLeagueReset", "bsSeasonEnd", "powerLeague", "crLeagueReset", "crSeasonEnd", "megaCrab", "tribeBoost", "bbSeasonEnd", "intelReset", "forcePointsDecrease"]) {
    ...
    if (element == "cwl") {
        ...
        if (Date.now() > thisMonthSeasonBegin.getTime()) {
            if (date.getUTCMonth() == 11) date.setUTCFullYear(date.getUTCFullYear() + 1);
            date.setUTCMonth(date.getUTCMonth() == 11 ? 0 : date.getUTCMonth() + 1);
        } else date = thisMonthSeasonBegin;
    } else if (element == "cocLeagueReset") {
        active = true;
        const thisMonthLastMonday = getLastMonday(date.getUTCMonth(), date.getUTCFullYear());
        if (Date.now() > thisMonthLastMonday.getTime()) date = getLastMonday(date.getUTCMonth() == 11 ? 0 : date.getUTCMonth() + 1, date.getUTCMonth() == 11 ? date.getUTCFullYear() + 1 : date.getUTCFullYear());
        else date = thisMonthLastMonday;
        ...
        if (date.getUTCMonth() == 11) nextMonthCGBegin.setUTCFullYear(date.getUTCFullYear() + 1);
    ...
    //Boom Beach
        ...
        if (tribeBoostReset.getTime() > megaCrabBegin.getTime() && megaCrabEnd.getTime() > tribeBoostReset.getTime()) date = megaCrabEnd;
...
function getLastMonday(month: number, year: number) {
    ...
    if (month == 11) date.setUTCFullYear(date.getUTCFullYear() + 1);
