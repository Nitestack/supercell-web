$.ajax({
    ...
    success: function (townHalls) {
        if (document.getElementById("BuildersHut1slideGroup")) for (let i = 3; i <= 5; i++) $(`#BuildersHut${i}slideGroup`).hide();
        ...
        //@ts-ignore
        function replaceAllUpperCase(text: string) {
            const arrayOfText = text.split(" ");
            let allUpperCase: boolean = true;
            for (const letter of arrayOfText) if (letter != letter.toUpperCase()) allUpperCase = false;
            if (allUpperCase) {
        ...
        $(".slider").on("input", function () {
            ...
            if (this.id.includes("Wall")) updateWallCount();
            //@ts-ignore
            else editImageElement(this.id, document.getElementById(this.id).value);
        });
        ...
        $(".form-control").on("input", function () {
            ...
            if (id.startsWith("number")) {
                ...
                //@ts-ignore
                else if (parseInt(numberElement.value.toString()) > sliderElement.max) return;
                //@ts-ignore
                else if (parseInt(numberElement.value.toString()) < sliderElement.min) return;
                //@ts-ignore
                ...
                if (id.toLowerCase().includes("builder")) {
                    ...
                    if (id.includes("Wall")) updateWallCount();
                    //@ts-ignore
                    else editImageElement(sliderElement.id, sliderElement.value);
                };
            } else if (id.startsWith("global")) {
                const globalElement = document.getElementById(id);
                //@ts-ignore
                if (globalElement.value == "") return;
                for (let i = 1; i; i++) {
                    ...
                    //@ts-ignore
                    if (parseInt(globalElement.value.toString()) > element.max) continue;
                    //@ts-ignore
                    else if (parseInt(globalElement.value.toString()) < element.min) continue;
                    //@ts-ignore
        ...
        //@ts-ignore
        function editImageElement(id: string, value: number | string) {
            if (id.toLowerCase() == "builder") return;
        ...
        //@ts-ignore
        function convertName(name: string, counter: number): string {
        ...
        document.getElementById("resetButton").onclick = function (ev) {
            for (const element of $(".slider").toArray()) {
                if (element.id.toLowerCase() == "builder") continue;
                //@ts-ignore
        ...
        document.getElementById("maxButton").onclick = function (ev) {
            ...
            if (number == 0 || townHalls.length < number) return;
            for (const element of $(".slider").toArray()) {
                if (element.id.toLowerCase() == "builder") continue;
                if (element.getAttribute(`max-${number}`)) setLevel(element, parseInt(element.getAttribute(`max-${number}`)));
                else if (element.id.includes("BuildersHut") && number != 14) setLevel(element, 1);
                else setLevel(element, 0);
            };
            updateWallCount();
        };

        //@ts-ignore
        function setLevel(element: HTMLElement, value: number) {
            //@ts-ignore
            element.value = value;
            //@ts-ignore
            document.getElementById("number" + element.id).value = value;
            if (!element.id.includes("Wall")) editImageElement(element.id, value);
        };
        //@ts-ignore
        ...
        function showBuilderHuts(amount: number) {
            if (document.getElementById("BuildersHut1slideGroup")) {
                for (let i = 1; i <= 5; i++) $(`#BuildersHut${i}slideGroup`).hide();
                for (let i = 1; i <= amount; i++) $(`#BuildersHut${i}slideGroup`).show();
            };
        };
    },
    error: function () {
